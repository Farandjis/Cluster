...
router.post('/execute_prime', async (req, res) => {
    ...
    if (dbUser == undefined) {
        return res.redirect('/inscription_connexion');
    }
    ...
    exec(command, { shell: true }, async (error, stdout, stderr) => {
        ...
        output_html = `
            ...
            <p>Find all primes up to: ${data.find_up_to}</p>
            <p>Nodes: ${data.nodes}</p>
            <p>Time elapsed: ${data.time_elapsed} seconds</p>
            <p>Primes found: ${data.primes_found}</p>
            <h2>All primes:</h2>
...
router.post('/execute_montecarlo', async (req, res) => {
    ...
    exec(command, { shell: true }, async (error, stdout, stderr) => {
        ...
        const output_html = `
            <h1 id="title_res">${data.message}</h1>
            <p>Total throws: ${data.nb_it}</p>
            <p>Number of processes: ${data.nodes}</p>
            <p>Estimated Pi: ${data.approx_pi}</p>
